...
export function AppSidebar({ email, role }: { email?: string; role?: string }) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={`
                fixed md:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm
                transition-transform duration-300 ease-in-out
                ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
            `}>
                <div className="h-16 hidden md:flex items-center px-6 border-b border-slate-100 bg-white">
                    <Link href="/app" className="flex items-center gap-2 group">
                        ...
                        <span className="text-lg font-bold text-slate-950 tracking-tight">VerifyIQ</span>
                    </Link>
                </div>
                ...
                <div className="p-4 border-t border-slate-100 flex flex-col gap-3 bg-slate-50/50">
                    <p className="text-xs text-slate-500 font-medium truncate px-2" title={email}>
                        {email}
                    </p>
                    <Link href="/api/auth/signout" className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-rose-600 hover:bg-rose-50 transition border border-transparent hover:border-rose-100">
